...
app.post('/register', async (req, res) => {
  ...
  app.post('/ologin', async (req, res) => {
    try {
      ...
      // Retrieve orphanage user by email using the admin SDK
      const orphanageRecord = await admin.auth().getUserByEmail(email);
  
      if (orphanageRecord) {
        // Retrieve orphanage data from Firestore
        const orphanageDocRef = admin.firestore().collection('orphanages').doc(orphanageRecord.uid);
        const orphanageDoc = await orphanageDocRef.get();
  
        if (orphanageDoc.exists) {
          // Retrieve hashed password from Firestore
          const storedHashedPassword = orphanageDoc.data().password;
  
          // Verify the entered password with the stored hashed password
          const isPasswordValid = await bcrypt.compare(password, storedHashedPassword);
  
          if (isPasswordValid) {
            // Generate JWT token with orphanage UID and email
            const token = jwt.sign(
              { uid: orphanageRecord.uid, email: orphanageRecord.email },
              'your-secret-key',
              {
                // Add token options if needed
              }
            ...
            // Include the token in the response header and respond with orphanage data
            res.header('Authorization', `Bearer ${token}`);
            res.json({
              message: 'Orphanage login successful',
              orphanageData: { email: orphanageRecord.email, uid: orphanageRecord.uid, token: token },
            });
          } else {
            res.status(401).json({ message: 'Invalid email or password' });
          }
        } else {
          res.status(404).json({ message: 'Orphanage not found in Firestore' });
        }
      } else {
  ...
  app.post('/nregister', async (req, res) => {
    try {
      ...
      res.json({ message: 'NGO registration successful', uid: nUid });
    } catch (error) {
      console.error('Error in oregister route:', error);
      res.status(500).json({ message: 'Internal Server Error' });
    }
...
app.post('/submitRequest', async (req, res) => {
  try {
    ...
    // Fetch user email from Firestore based on UID
    const userDoc = await admin.firestore().collection('users').doc(uid).get();
    console.log(req.body,"outside the submit route")
    if (userDoc.exists) {
      donorEmail = userDoc.data().email;

      // Perform necessary actions with the form data
      // For example, you can send an email, store the data in a database, etc.
      console.log(email,donorEmail,"submit route")
      // Log the form data along with the user's email
      console.log('Received form data:', { userEmail, subject, message });

      const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
          user: 'annapurna2005k@gmail.com', // replace with your Gmail email
          pass: 'byvjnecnxhxrivnw' // replace with your Gmail password or an app-specific password
        }
      });
      const htmlBody = `
        <!DOCTYPE html>
